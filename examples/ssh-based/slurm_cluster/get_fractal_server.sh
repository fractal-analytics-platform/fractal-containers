...
if [ -z "${FRACTAL_SERVER_RELEASE}" ]; then
    if [ -z "${FRACTAL_SERVER_GIT}" ]; then
        ...
        /fractal-server-venv/bin/python3 -m pip install "fractal-server"
    else
        # Case 2: only git set
        /fractal-server-venv/bin/python3 -m pip install "fractal-server @ git+https://github.com/fractal-analytics-platform/fractal-server.git@${FRACTAL_SERVER_GIT}"
        EXIT_CODE=$?
    ...
    if [ -z "${FRACTAL_SERVER_GIT}" ]; then
        ...
        /fractal-server-venv/bin/python3 -m pip install "fractal-server==$FRACTAL_SERVER_RELEASE"
    else
