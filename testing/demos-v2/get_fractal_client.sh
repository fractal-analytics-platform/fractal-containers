...
if [ -z "${FRACTAL_CLIENT_V2_RELEASE}" ]; then
    if [ -z "${FRACTAL_CLIENT_V2_GIT}" ]; then
        ...
        # Case 2: only git set
        fractal-tasks-venv/bin/python -m pip install git+https://github.com/fractal-analytics-platform/fractal-client.git@$FRACTAL_CLIENT_V2_GIT
        # Check exit code
        EXIT_CODE=$?
    ...
    if [ -z "${FRACTAL_CLIENT_V2_GIT}" ]; then
        ...
        fractal-tasks-venv/bin/python -m pip install fractal-client==$FRACTAL_CLIENT_V2_RELEASE
    else
