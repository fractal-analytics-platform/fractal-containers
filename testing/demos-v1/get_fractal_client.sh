...
if [ -z "${FRACTAL_CLIENT_V1_RELEASE}" ]; then
    if [ -z "${FRACTAL_CLIENT_V1_GIT}" ]; then
        ...
        # Case 2: only git set
        python3 -m pip install git+https://github.com/fractal-analytics-platform/fractal-client.git@$FRACTAL_CLIENT_V1_GIT
        # Check exit code
        EXIT_CODE=$?
    ...
    if [ -z "${FRACTAL_CLIENT_V1_GIT}" ]; then
        ...
        python3 -m pip install "fractal-client==$FRACTAL_CLIENT_V1_RELEASE"
    else
